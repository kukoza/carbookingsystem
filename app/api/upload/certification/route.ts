...
export async function POST(request: Request) {
  try {
    ...
    const fileName = `certification_${uuidv4()}.${fileExtension}`
    ...
    console.log("Certification image uploaded successfully:", {
      fileName,
      fileUrl,
      filePath,
    ...
    return NextResponse.json({ error: "Failed to upload certification image" }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to delete certification image" }, { status: 500 })
  }
}
