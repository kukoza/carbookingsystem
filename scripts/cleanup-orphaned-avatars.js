...
async function cleanupOrphanedAvatars() {
  ...
  try {
    ...
    const files = fs.readdirSync(UPLOAD_DIR).filter((file) => file !== ".gitkeep" && !file.startsWith("."))
    ...
    const [rows] = await connection.execute("SELECT avatarFileName FROM Users WHERE avatarFileName IS NOT NULL")
    const usedFiles = rows.map((row) => row.avatarFileName).filter(Boolean)
